...
- `NTFY_DIUN_TOKEN` - ntfy **access token** (not a password) used by the `diun` container to publish update notifications to `nas-alerts`. Minted for the write-only `arr` publisher with `docker exec ntfy ntfy token add --label "diun (update notifications)" arr`; revoke with `ntfy token remove arr <token>`. Diun's ntfy notifier supports a token only. See ADR-0024.
