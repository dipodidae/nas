...
- **The qBittorrent upload cap has two values, chosen by the shaper's state.** 25 Mbps while CAKE is installed (pure capacity — DSCP makes torrents yield), 15 Mbps when it is not (measured unshaped at 0% loss). `qbittorrent_settings_enforce.py` reads `tc qdisc show` and picks; it runs `*/5`, not hourly, because it is a safety net and an hour of unmanaged uplink is an hour of 5% loss. The watchdog observes, the enforcer acts — keep those separate.
