...
- **Bazarr history paths go stale on a Sonarr rename, and its sync API returns 204 before doing anything.** `/api/episodes/history` keeps the subtitle's filename from download time (`…S03E07.1080p.BluRay.x264-YELLOWBIRD.en.srt`) and uses the `/tv/` namespace, while `/api/episodes` reports `/data/series/…`. So a history row is found by episode + language, never by path (`subtitle_audit.latest_download`). `PATCH /api/subtitles?action=sync` and the blacklist endpoint's re-search go through Bazarr 1.6's **job queue**: `204` in 20 ms, and the work happens minutes later, or after the ~200 jobs a profile change enqueues. Read `/api/system/jobs`, not the status code. ADR-0054.
- **`*arr` "Update Library" notifications return 204 and can still do nothing.** Sonarr/Radarr/Lidarr's MediaBrowser connection reports the *arr's own path (`/tv`, `/movies`, `/music`); Jellyfin's libraries live under `/data/movies/{series,movies,music}`. `POST /Library/Media/Updated` answers 204 for a path under no Jellyfin library and drops it silently, so the connection looks healthy while nothing refreshes. Sonarr and Radarr have `mapFrom`/`mapTo` fields for this and they are now set (`/tv` -> `/data/movies/series`, `/movies` -> `/data/movies/movies`). **Lidarr has no Jellyfin connection any more (ADR-0051):** Jellyfin holds no music library, Navidrome does, and Lidarr's `Subsonic` connector fires `/rest/startScan` on import instead (ADR-0049). `scripts/lidarr_jellyfin_bridge.py` and its `2-59/5` cron are deleted. The Sonarr/Radarr mappings above are unchanged and still required.
- **Jellyfin is movies, series and (future) books — not music (ADR-0051).** Music was 99% of its items (168,128 `Audio` vs 1,789 `Episode`). Two traps if you ever remove a library: deleting it **orphans its items rather than deleting them**, and a library scan will not reap them (validation walks libraries; orphans sit under none) — `CleanDatabaseScheduledTask` does, and leftover `MusicGenre` items and playlists need deleting by id. And **SQLite does not shrink on delete**: with every music item gone the DB was still 1.426 GB at 94.2% free pages, and `VACUUM` took it to 69 MB. `make verify-runtime` asserts both, via `scripts/check-jellyfin-no-music.py`.
...
- **REQUIRED (host, if tinyauth is running)** `TINYAUTH_USER`, `TINYAUTH_PASSWORD_HASH` - the single credential for the whole protected public surface (ADR-0034). One user on purpose: one household, no directory. `TINYAUTH_PASSWORD_HASH` is **bcrypt and contains `$`** — mint it with `docker run --rm -it ghcr.io/tinyauthapp/tinyauth:v5.2.0 user create --interactive`. Neither variable is ever referenced from a compose file: `make tinyauth-users` renders them into `secrets/tinyauth-users` (mode `0600`, gitignored) which the container mounts **`:ro`** at `/secrets/users`, so the credential never enters an `environment:` block and never reaches `docker inspect` (ADR-0011), and cannot be rewritten by the container that reads it (ADR-0033). `make check` asserts all four properties plus that the rendered file still matches `.env` byte for byte — the `$` in the hash was already re-expanded once by Make (`$2: unbound variable`), so the rendering is not assumed to be safe, it is checked.
- **REQUIRED (AudioMuse-AI, ADR-0053)** `AUDIOMUSE_DB_PASSWORD` (compose-consumed, its Postgres), `AUDIOMUSE_ADMIN_USER` / `AUDIOMUSE_ADMIN_PASSWORD` (AudioMuse's own login, created once by `scripts/audiomuse_setup.py`), `AUDIOMUSE_API_TOKEN` (bearer token: the setup script and `check-navidrome-plugins.py` use it, and `make navidrome-plugins` writes it into the `audiomuseai` plugin's config), `NAVIDROME_AUDIOMUSE_USER` / `NAVIDROME_AUDIOMUSE_PASSWORD` - a dedicated **non-admin** Navidrome account AudioMuse streams every track as, over `navidrome:4533` only. Create it with Navidrome's native `POST /api/user` (`navidrome user create` needs a TTY for the password).
- **REQUIRED (scripts, if the Lidarr → Navidrome connector is wired)** `NAVIDROME_LIDARR_USER`, `NAVIDROME_LIDARR_PASSWORD` - a **dedicated** Navidrome account that Lidarr's `Subsonic` connector authenticates as to call `/rest/startScan` on every import, so a new album does not wait up to an hour for `ND_SCANNER_SCHEDULE`. It **must be a Navidrome admin**: 0.64 gates `startScan` on `is_admin` and ships no scan-only role, so a demoted account gets Subsonic error `50` while `ping` and `getScanStatus` keep answering `200` — silent, which is why `make verify-runtime` probes `getUser` for `adminRole`. Create with `docker exec -it navidrome navidrome user create -u lidarr` then `docker exec navidrome navidrome user edit -u lidarr --set-admin`. Lidarr sends the password as a **plaintext `p=` query parameter**, so never reuse a password here and keep the connector pointed at the container name `navidrome:4533` with SSL off — never at `navidrome.${PUBLIC_DOMAIN}`, whose `/rest` is deliberately un-gated for phone clients (ADR-0044). See ADR-0049.
