...
- **The media drive has no SMART and no redundancy.** `/mnt/drive` is a single 9.1 TB USB external disk holding everything; its bridge refuses SMART under every `smartctl -d` type (sat, sat,12, sat,16, usbjmicron, usbsunplus, usbcypress, scsi — all verified 2026-09-02). The only early warning is the kernel log, so `stack_watchdog.py` watches for I/O errors, USB resets, an ext4 read-only remount and the mount disappearing. **A read-only remount is the one to fear**: ext4 does it on error by default, and every *arr import then fails while every container still reports healthy.
- **When a check passes, ask whether it proves the property you care about or just the component that carries it.** This bit three times in one investigation: a `LibraryMonitor` line proved *a* refresh but not what caused it; a cgroup match proved the container but not the process; `tc qdisc show | grep cake` proved CAKE was loaded but not that egress was still prioritised. Make the thing answer for its own behaviour — `scripts/wan_shaper.sh check` verifies qdisc *and* rate *and* DSCP marks, and every caller asks it rather than grepping `tc` and drawing its own conclusion. Test for writing a check: name the failure you are guarding against, then ask whether your check would still pass during it.
...
- **Two ntfy topics, and the split is load-bearing.** `nas-alerts` is things you must act on (health issues, failures, manual interaction, watchdog, Watchtower); `nas-media` is things that are merely nice to know (imports, upgrades, subtitles). Routing informational events at `nas-alerts` would bury the failures the alerting exists to surface — which is exactly the problem it was built to fix. **Lidarr deliberately publishes nothing to `nas-media`**: this box does hundreds of music imports a day and they would drown everything.
- **Some services cannot set an auth header; ntfy has two ways round it.** `?auth=<base64url("Basic "+base64(user:pass))>` as a query parameter (used by Jellyseerr's webhook), or `user:pass@host` URL userinfo (used by Watchtower's shoutrrr URL and Bazarr's Apprise URL). Both verified working against this deny-all instance.
...
- `NTFY_ADMIN_USER`, `NTFY_ADMIN_PASSWORD` - ntfy admin, for the web UI and for managing the other accounts.
- `NTFY_ARR_USER`, `NTFY_ARR_PASSWORD` - **write-only** publisher used by the *containers* (Sonarr/Radarr/Lidarr/Prowlarr/Jellyseerr/Bazarr/Watchtower), separate from `watchdog` because they are a different trust domain — they talk to indexers and trackers — and can be revoked without touching host alerting.
