...
- `NTFY_ARR_USER`, `NTFY_ARR_PASSWORD` - **write-only** publisher used by the _containers_ (Sonarr/Radarr/Lidarr/Prowlarr/Jellyseerr/Bazarr/Watchtower), separate from `watchdog` because they are a different trust domain — they talk to indexers and trackers — and can be revoked without touching host alerting.
