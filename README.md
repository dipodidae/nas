...
| Services        | 35 (4 built locally; ALL watched by diun, 13 manual-update)     |
...
| `adguardhome`  | adguard/adguardhome **pinned** | `192.168.2.56:53`, `127.0.0.1:3053` | **NO** | LAN + WireGuard DNS filter. Binds **one** address; the host does not use it [ADR-0043](docs/decisions/0043-adguard-home-lan-dns.md)     |
...
| Service        | Image                                      | Ports                                  | wt     | Notes                                                                                                                                                                                                                                         |
| -------------- | ------------------------------------------ | -------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `jellyfin`     | lscr.io/…/jellyfin **pinned**              | `8096`, `8920`, `7359/udp`, `1900/udp` | **NO** | Movies + series only — **no music** ([ADR-0051](docs/decisions/0051-jellyfin-drops-music.md)); Navidrome owns that. QSV via `/dev/dri`. `mem_limit 10g` + 2 leak mitigations → [ADR-0008](docs/decisions/0008-jellyfin-memory-mitigations.md) |
| `jellyseerr`   | ghcr.io/fallenbagel/jellyseerr             | `127.0.0.1:5056`                       | yes    | Requests                                                                                                                                                                                                                                      |
| `navidrome`    | deluan/navidrome **pinned**                | `127.0.0.1:4533`                       | **NO** | Subsonic music server. `/rest` path-scoped **open** ([ADR-0044](docs/decisions/0044-navidrome-subsonic-path-scope.md)); plugins via `make navidrome-plugins` ([ADR-0053](docs/decisions/0053-navidrome-plugins-and-audiomuse.md))             |
| `audiomuse`    | ghcr.io/neptunehub/audiomuse-ai **pinned** | `127.0.0.1:8010`                       | **NO** | Sonic analysis; answers Navidrome Instant Mix. + `audiomuse-worker{,-2}`, `audiomuse-db`. Config via `make audiomuse-setup` ([ADR-0053](docs/decisions/0053-navidrome-plugins-and-audiomuse.md))                                              |
...
sample. The two forward-auth confs (`swag/tinyauth-server.conf`,
`swag/tinyauth-location.conf`) are tracked the same way even though they live
at `/config/nginx/` rather than `proxy-confs/`; the copy that was live in the
container was five revisions behind the image's own sample and missing the
identity headers. → [ADR-0022](docs/decisions/0022-proxy-confs-are-tracked.md),
[ADR-0034](docs/decisions/0034-one-door-tinyauth.md)

**A conf edit is a deploy, and the way to apply one is `make swag-apply`.**
Docker binds each conf by **inode**, so anything that replaces the host file
rather than rewriting it in place — `git checkout`, `git revert`, `git stash
pop`, prettier, `sed -i`, most editors — silently detaches the mount. nginx
keeps serving the old file while `git diff` is clean, `nginx -t` passes and
`nginx -s reload` changes nothing. `make verify-runtime` compares every tracked
conf's sha256 against what the container holds and escalates a mismatch to
`nas-critical`.

| URL                                                                      | Service                                                      | Door                                                               |
| ------------------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------ |
| `4eva.me`                                                                | `4eva-rootpage` (apex, via the mounted `root.conf`)          | **public**                                                         |
| `4eva.me/ops.html` + `/ops-status.json`                                  | Live stack dashboard, fed by `media_ops_status.py`           | tinyauth, path-scoped                                              |
| `auth.`                                                                  | `tinyauth` itself — the login page                           | **public**, necessarily                                            |
| `sonarr.` · `radarr.` · `lidarr.` · `bazarr.` · `prowlarr.` · `lingarr.` | The \*arr suite                                              | tinyauth on `location /`; `/api` stays on the API key              |
...
| `ntfy.`                                                                  | Push alerts                                                  | **never** — a door on the alert channel makes a broken door silent |
...
answers. → [ADR-0034](docs/decisions/0034-one-door-tinyauth.md)
...
| Every `protect` route includes the auth conf in `location /`, and no `never` route includes it at all                                                                                                             | A route that silently loses its door answers `200` to the whole internet and nothing else notices. Both directions are asserted, and `make verify-runtime` re-checks the live answers — a `500` there means the door is JAMMED SHUT (tinyauth unreachable), not open                  | [0034](docs/decisions/0034-one-door-tinyauth.md)                                                                                                             |
| `tinyauth` keeps `LABELPROVIDER=none`, no Docker socket and no dockerproxy route                                                                                                                                  | Its default is `auto`, which discovers per-app ACLs from Docker **labels** and therefore needs the socket. Per-app rules are `TINYAUTH_APPS_[NAME]_*` variables instead                                                                                                               | [0013](docs/decisions/0013-dockerproxy-sole-socket-holder.md), [0034](docs/decisions/0034-one-door-tinyauth.md)                                              |
| The tinyauth credential is a `0600` file mounted `:ro`, matching `.env` byte for byte — and `TINYAUTH_PASSWORD_HASH` stays **single-quoted** in `.env`                                                            | An env var leaks into `docker inspect`; a writable credential file lets a container escalate its own access. The quoting is not cosmetic: `.env` is `.`-sourced under `set -u`, where an unquoted `$2a$10$…` aborts with `$2: unbound variable` and silently skips every later check  | [0011](docs/decisions/0011-qbittorrent-credentials.md), [0033](docs/decisions/0033-ntfy-topic-taxonomy.md), [0034](docs/decisions/0034-one-door-tinyauth.md) |
| No live `auth_basic` in any tracked conf, and no retired per-app auth variable anywhere                                                                                                                           | `auth_basic` does not stack with forward auth, it **preempts** it: its `401` is what `error_page 401` converts, so the auth subrequest never runs and a valid session is locked out. Measured on ongehoord                                                                            | [0034](docs/decisions/0034-one-door-tinyauth.md)                                                                                                             |
| A tracked proxy-conf edit is applied with `make swag-apply`, never `nginx -s reload`                                                                                                                              | Docker binds each conf by **inode**, so `git checkout`/`revert`/prettier/`sed -i` detach the mount: nginx serves the old file while `git diff` is clean and `nginx -t` passes. `make verify-runtime` compares sha256s and escalates a mismatch to `nas-critical`                      | [0022](docs/decisions/0022-proxy-confs-are-tracked.md), [0034](docs/decisions/0034-one-door-tinyauth.md)                                                     |
...
- **Tier-3 items from the auth migration, deliberately not built:** audit-stream
  ...
  _properly_ instead of being excluded. See the end of
  [ADR-0034](docs/decisions/0034-one-door-tinyauth.md).
- ~~**`pnpm lint` is red on three files this work did not touch.**~~ **Closed
  2026-09-04 by `fc27592`** ("style: format the report's tables"), which ran
  `lint:fix` over `TRIAGE-2026-09-03.md`,
  `docs/decisions/0032-alert-noise-ownership.md` and
  `docs/removed-indexers/torrent-core-2026-09-03.json`. The entry outlived the
  fix by one commit; verified green by stashing the working tree and running
  `pnpm lint` against `HEAD`. Kept here, struck through, because "the lint gate
  is red and that's fine" is exactly the belief that lets a real failure hide.
