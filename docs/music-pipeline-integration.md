...
_Generated by `scripts/gen_pipeline_tables.py` from `cron/crontab`. 33 scheduled jobs._
...
| `45 5 * * 0`          | `jellyfin-image-backfill`  | `jellyfin_image_backfill.py`      | Backfill missing Jellyfin artist and album images from the enabled providers   | `0,1 (default)` |
...
| `20 5 * * 0`          | `jellyfin-scan-music`      | `jellyfin_library_scan.py`        | Scan a single Jellyfin library (or all of them) via the API                    | `0,1 (default)` |
...
| `/tmp/nas-jf-image-backfill.lock`    | 1       | — (all non-blocking)                                            |
...
**`--ok-codes`.** 6 of 33 jobs declare it explicitly; 27 inherit the `0,1` default, which swallows exit 1.
