...
it at `${CONFIG_DIRECTORY}/cleanuparr/blacklist.txt`; it is intentionally _not_
...
- `deleteIfAnyFileBlocked: true` — the fake-release pattern always bundles the
  executable _alongside_ a decoy video, so per-file blocking would never reach
  it.
...
on an incomplete download. This matters more than it looks: a _completed_
...
ntfy at `http://ntfy:8410`, topic **`nas-cleanuparr`** — deliberately _not_
...
The cause is that "no seeders" means _no other seeder is online_, which is the
