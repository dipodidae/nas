...
- **Auto-update path: `watchtower` + `dockerproxy`.** Watchtower runs on the schedule in `WATCHTOWER_SCHEDULE` (default `0 0 4 * * *` — daily at 04:00) and only acts on containers carrying `com.centurylinklabs.watchtower.enable=true`. It talks to Docker through `dockerproxy` (tecnativa/docker-socket-proxy) — never mount `/var/run/docker.sock` into any other service. Locally-built images (`lidarr-bulk`, `4eva-rootpage`) are deliberately unlabeled; `watchtower`/`dockerproxy` themselves are unlabeled too (Watchtower shouldn't self-update or restart its own dependency). **`qbittorrent` and `jellyfin` are also deliberately unlabeled as of 2026-09-01.** Watchtower's stop→remove→create is *not atomic*: when the remove fails it logs `Failed=1` and moves on **without creating a replacement**, leaving no container at all — `restart: unless-stopped` cannot help, and autoheal cannot heal what does not exist. That is not theoretical; it deleted qbittorrent for 13h on 2026-09-01. The risk scales with how slow a service is to stop, so slow, important services should be updated by a deliberate `docker compose pull <svc> && docker compose up -d <svc>` rather than on a schedule. `scripts/stack_watchdog.py` (cron `*/5`) is the detector for the general case — it alerts when a service defined in the compose file has no container at all, not merely when one is unhealthy.
