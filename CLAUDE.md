...
- **qBittorrent crash-loop after config/network changes → stale lockfiles (now self-healing).** Root cause: qbit shares gluetun's netns (`network_mode: service:gluetun`), so a gluetun recreate (compose up/update, watchtower, VPN reconnect) force-restarts qbit; if it's SIGKILLed before flushing its ~6 MB `torrents.db`+WAL it leaves an orphaned `${CONFIG_DIRECTORY}/qbittorrent/qBittorrent/lockfile` (and historically a nested `qBittorrent/config/lockfile`). On next start qBittorrent's single-instance lock refuses to launch, `qbittorrent-nox` exits immediately, and s6 restarts it in a tight loop — the only tell is rapidly incrementing PIDs in the log, while the container reports "Up (unhealthy)". **This is NOT an ownership/permissions problem** (config is correctly `${PUID}:${PGID}`). Two durable fixes are now in compose: (1) `stop_grace_period: 120s` so qbit shuts down cleanly and removes its own lockfile; (2) an LSIO custom-init script (`qbittorrent/custom-cont-init.d/01-clear-stale-lockfile.sh`, bind-mounted `:ro` to `/custom-cont-init.d`) that deletes any stale lockfile at container init — safe because qbit isn't running yet, so the container self-recovers on every restart. Manual fallback if ever needed: `docker stop qbittorrent && rm -f ${CONFIG_DIRECTORY}/qbittorrent/qBittorrent{,/config}/lockfile && docker start qbittorrent`.
