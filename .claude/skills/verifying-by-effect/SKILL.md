...
| A `401` from tinyauth's login                                        | "wrong password"          | It also means **the account is LOCKED** (3 failures / 300s, in memory) -- the correct password gets `401` too. The attempt that trips the lock returns `401`, so a `401` never proves the limiter is clear. `make tinyauth-unlock`. |
