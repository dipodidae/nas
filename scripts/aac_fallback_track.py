...
def main(argv: list[str] | None = None) -> int:  # noqa: PLR0911
  ...
  for path in candidates:
    ...
    print(f"{'convert' if args.apply else 'DRY-RUN'}  {path.name} "
          f"(default {default.codec} {default.channels}ch -> +aac 2ch default)")
    if not args.apply:
      converted += 1
      continue
    ...
    try:
      proc = subprocess.run(build_command(ffmpeg, path, staged, p, args.bitrate),
                            capture_output=True, text=True, timeout=7200, check=False)  # fmt: skip
