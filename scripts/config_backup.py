...
DEFAULT_EXCLUDES = [
  ...
  # Scrubbing-preview tiles: ~3.8 MB per 45-min episode, ~6 GB for this library, and
  # regenerated by the nightly "Generate Trickplay Images" task. They normally live
  # next to the media on ${SHARE_DIRECTORY} (ADR-0039), which this archive does not
  # cover at all -- this entry is the guard for the day someone unticks "save with
  # media", which silently redirects 6 GB into /config on a 76%-full root. Note the
  # path: they land under data/data/, NOT the data/metadata/ excluded above.
  "jellyfin/data/data/trickplay/**",
