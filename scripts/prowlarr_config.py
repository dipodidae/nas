...
class ProwlarrConfig:
    ...
    def get_priority(self, indexer_name: str) -> Optional[int]:
    ...
    def display_summary(self) -> None:
        ...
        priority_ranges = {
            "High Priority (1-10)": [p for p in priority_groups.keys() if 1 <= p <= 10],
            "Mid Priority (11-30)": [p for p in priority_groups.keys() if 11 <= p <= 30],
            "Standard Priority (31-40)": [p for p in priority_groups.keys() if 31 <= p <= 40],
            "Low Priority (41-50)": [p for p in priority_groups.keys() if 41 <= p <= 50],
        }
...
def load_prowlarr_config(config_path: Optional[str] = None) -> ProwlarrConfig:
