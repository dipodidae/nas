...
class IndexerMatcher:
    ...
    @staticmethod
    def find_best_match(target_name: str, available_names: List[str],
                       threshold: float = FUZZY_MATCH_THRESHOLD) -> IndexerMatchResult:
...
class ErrorClassifier:
    ...
    def classify_error(status_code: int, response_text: str, indexer_name: str) -> Tuple[str, str]:
        ...
        if status_code == 400:
            if "priority" in response_lower and ("between" in response_lower or "range" in response_lower):
    ...
    def fetch_indexers(self) -> List[Dict[str, Any]]:
        ...
        for endpoint in indexer_endpoints:
            try:
                ...
                print(f"Timeout error for {endpoint}: Request timed out after {REQUEST_TIMEOUT_SECONDS} seconds")
                print(f"Error details: {e}")
                continue
                ...
                try:
                    ...
                    if response.status_code in [200, 202]:
                        ...
                        if attempt < MAX_RETRY_ATTEMPTS - 1:
                            print(f"Database locked, retrying in {RETRY_DELAY_SECONDS} seconds... (attempt {attempt + 1}/{MAX_RETRY_ATTEMPTS})")
                        ...
                        error_type, error_description = ErrorClassifier.classify_error(
                            response.status_code, response.text, indexer_name
                        )
...
class IndexerPriorityUpdater:
    ...
    def _process_indexers(self, indexers: List[Dict[str, Any]]) -> None:
        ...
        for indexer in indexers:
            ...
            match_result = self.matcher.find_best_match(
                indexer_name, available_priority_names
            )
    ...
    def _process_matched_indexer(self, indexer: Dict[str, Any],
                               match_result: IndexerMatchResult,
        ...
        if match_result.confidence < 1.0:
            print(f"📍 Fuzzy matched '{indexer_name}' to '{matched_name}' "
    ...
    def _update_indexer_priority(self, indexer: Dict[str, Any],
                               new_priority: int, base_url: Optional[str]) -> None:
    ...
    def _display_summary(self, total_indexers: int) -> None:
        ...
        if self.summary.results['indexer_issues']:
            ...
            for item in self.summary.results['indexer_issues']:
                print(f"  - {item}")
            print("  💡 These are indexer-specific problems, not script failures. Check indexer settings in Prowlarr UI.")
        ...
        real_failures = (len(self.summary.results['database_locked']) +
